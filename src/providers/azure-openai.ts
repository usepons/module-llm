import OpenAI, { AzureOpenAI } from 'npm:openai@^4';
...
export interface AzureOpenAIConfig extends ProviderConfig {
  ...
  apiVersion?: string;
  /** Default deployment name to use when model is not specified */
  deployment?: string;
}
...
export class AzureOpenAIProvider implements LLMProvider {
  ...
  constructor(config: AzureOpenAIConfig) {
    ...
    if (!endpoint) {
      throw new Error('Azure OpenAI requires an endpoint (e.g. https://my-resource.openai.azure.com)');
    }

    this.endpoint = validateProviderBaseUrl(endpoint);
  ...
  async generateText(options: GenerateOptions): Promise<GenerateResult> {
    const response = await this.client.chat.completions.create({
      model: this.resolveDeployment(options.model),
  ...
  async listModels(): Promise<ModelInfo[]> {
    try {
      ...
      // Azure may not support listing models via API — return empty
      return [];
    }
  }
  ...
  private resolveDeployment(model: string): string {
    return model || this.defaultDeployment || '';
  }

  private buildMessages(options: GenerateOptions): OpenAI.ChatCompletionMessageParam[] {
